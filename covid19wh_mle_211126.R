...
# qm = 0.6297
# r = 0.1
# In the following, we donot use the parameter values in Tang's paper, instead we try to estimate the exponential parameters by MLE
...
# quarantine rate function with the same exponential structure as Tang
quar_rate <- function(t, q0, qm, r){
  qt = (q0-qm)*exp(-r*t) + qm
  return(1-qt)
}

######### 
## consider the binomial distribution to calculate the confidence interval

(T1_num)
(T2_num)
(epi_day <- df_wh$Day_num)
i.mu <- rep(0, T2_num - T1_num + 1)
(inc <- df_wh$Daily_case)
inc_uq <- rep(0.1, length(inc))
inc_uq[1:(T1_num-1)]<- inc[1:(T1_num-1)] # no quarantine restriction prior to T1

####
R0 =  6.0  # adjust for comparison
####
(sample_num = T2_num - T1_num + 1)
MLE_prob_exp = function(param){
  q0 = exp(param[1])/(1 + exp(param[1]))
  qm = exp(param[2])/(1 + exp(param[2]))
  r = param[3]
  # (R0 = param[4])
  inc_uq[1:(T1_num-1)]<- inc[1:(T1_num-1)] # no quarantine restriction prior to T1
  for(i in T1_num:T2_num){
    inc_uq[i] <- rbinom(n = 1, 
                        size = inc[i],
                        prob = quar_rate(i-T1_num, q0, qm, r)
                        ) # cases missing from quarantine prior to T1
  } 
  for(t in T1_num:T2_num){
    renew <- R0*sum(inc_uq[(t-1):1] * gam((1:(t-1)), shap, rat))
    i.mu[t-T1_num+1] <- max(round(renew), 0.1) # here we only consider the expected case after T1_num
  }
  negloglik = - sum(inc[T1_num:T2_num]*log(i.mu) - i.mu)
  nll = negloglik
  param_num = length(param)
  (AIC_exp = 2 * param_num + 2 * nll) 
  (AICc_exp = AIC_exp + (2*param_num^2 + 2*param_num)/(sample_num-param_num-1))
  
  return(AICc_exp)
}
param0 = c(0.05, 0.95, 0.1)   # the first three is exponential params of quarantine rate; the last is the R0
param_num = length(param0)
sample_num = T2_num - T1_num + 1
(AICc_exp = MLE_prob_exp(param0))
#(AIC_exp = 2 * param_num + 2*nll) 
#(AICc_exp = AIC_exp + (2*param_num^2 + 2*param_num)/(sample_num-param_num-1))

#
# define the initial dataframe of quarantine rate function parameter
df.par_exp_est <- data.frame(
  q0_est = numeric(0),
  qm_est = numeric(0), 
  r_est = numeric(0),
  R0 = numeric(0),
...
nsample = 500 # the number of re-samples
n=1
while(n < nsample){
  param0 = c(0.05, 0.95, 0.1) # q_0,q_m,r
  opt_exp <- optim(
  #opt_exp <- nmkb(
    par = param0,
    MLE_prob_exp,
    ...
    method = "SANN" 
    #method = "L-BFGS-B" 
    #hessian=FALSE # use numDeriv package to estimate the gradient and Hessian at the point of convergence
  )
  (q0_est = exp(opt_exp$par[1])/(1 + exp(opt_exp$par[1])))
  (qm_est = exp(opt_exp$par[2])/(1 + exp(opt_exp$par[2])))
  #(q0_est = opt_exp$par[1])
  #(qm_est = opt_exp$par[2])
  (r_est = opt_exp$par[3])
  # (R0_est = opt_exp$par[4])
  
  param_num = length(param0)
  # sample_num = T2_num - T1_num + 1
  (AICc_exp = opt_exp$value)
  #(AIC_exp = 2*param_num + 2*nll) 
  #(AICc_exp = AIC_exp + (2*param_num^2 + 2*param_num)/(sample_num-param_num-1))
  
  # if (
  #   ((abs(q0_est) < 1e-3) & (abs(qm_est-1) < 1e-3)) |  (abs(qm_est) < 1e-3) | (abs(r_est) < 1e-3) |
  #     (abs(q0_est) > abs(qm_est))
  #     )
  # {print(n);next}
  
  df.app <- data.frame(q0_est, qm_est, r_est, R0, AICc_exp)
  df.par_exp_est <- rbind(df.par_exp_est, df.app)
  n = nrow(df.par_exp_est)
  print(n)
}

round(df.par_exp_est, 2)

# save estimation result to csv
write.csv(df.par_exp_est, 
          "/Users/baoyinyuan/Documents/postdoc/research/EndofEpidemic/data_R6.0/par_exp_est.csv", 
          row.names = FALSE)
df.par_exp_est <- read.csv(file="/Users/baoyinyuan/Documents/postdoc/research/EndofEpidemic/data_R3.0/par_exp_est.csv")


(t_len <- (T2_num - T1_num + 1))
quar_rate_sample = data.frame(matrix(rep(0, nsample * t_len), ncol = nsample))
for (i in 1:nsample){
  (q0_sam <- df.par_exp_est[i, 1])
  (qm_sam <- df.par_exp_est[i, 2])
  (r_sam <- df.par_exp_est[i, 3])
  quar_rate_sample[,i] <- quar_rate(1:t_len, q0_sam, qm_sam, r_sam) # calculated quarantine rate
}

ci_value = c(0.025,0.975, 0.5) # 95% CI
# ci_value = c(0.05,0.95, 0.5) # 90% CI
ci_name =c("lower_exp","upper_exp","median_exp")
nci = length(ci_value)

CI_qr = data.frame(matrix(rep(0, nci*t_len), ncol = nci))
colnames(CI_qr) <- ci_name

for (j in 1:t_len){
  CI_qr[j,] = as.numeric(quantile(quar_rate_sample[j,], probs = ci_value))
} 
CI_qr

#R0_exp_ci = as.numeric(quantile(df.par_exp_est$R0_est, probs = ci_value)) # confidence interval of R0
#R0_exp_ci 

# plot the q_t with confidence interval 
plot(CI_qr$upper_exp, type="l",col="red",ylim=c(0,1))
lines(CI_qr$lower_exp, col="green")
lines(CI_qr$median_exp,col="blue")


library("magrittr")
library(dplyr)
# add 46 rows NA values to the CI_qr dataframe
na.mat <- matrix(NA, nrow = T1_num-1, ncol=3)
colnames(na.mat) <- colnames(CI_qr)
CI_qr1.na <- rbind(na.mat, CI_qr)
CI_qr1.na$day_num <- 1:T2_num
CI_qr1.na$date <- df_wh$Day_report[1:T2_num]
CI_qr1.na$inc <- df_wh$Daily_case[1:T2_num]
...
####
# R0 = 2.0 # can be adjusted
####

### for each R0, we change num.knots
num.knots <- 40 ## can be changed. Default as 8 for the first draft!!
###
...
#(AIC_ls= 2*(param_num_ls) + 2*nll) # AIC value 
#(AICc_ls = AIC_ls + (2*param_num_ls^2 + 2*param_num_ls)/(sample_num_ls - param_num_ls - 1))
